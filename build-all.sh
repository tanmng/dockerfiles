...
# build-all.sh
#
# A script that build all the Dockerfile and push them
# Credit to Jessie Frazelle and her awesome script (https://github.com/jessfraz/dockerfiles/blob/master/build-all.sh)
set -e
set -o pipefail

SCRIPT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/$(basename "${BASH_SOURCE[0]}")"
REPO_URL="${REPO_URL:-nmtan}"
JOBS=${JOBS:-2}
...
main(){
	# get the dockerfiles
	IFS=$'\n'
	files=( $(find . -iname '*Dockerfile' -maxdepth 2 | sed 's|./||' | sort) )
	unset IFS
